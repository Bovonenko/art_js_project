...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_filter__WEBPACK_IMPORTED_MODULE_8__["default"])('.portfolio-menu', 'li', '.portfolio-wrapper', '.all', '.portfolio-no');
...
var calc = function calc(size, material, options, promocode, result) {
  ...
  var sum = 0,
      ...
      optionsValue = '0';
  var state;
  Object(_services_request__WEBPACK_IMPORTED_MODULE_0__["getResource"])('assets/calcPrice.json').then(function (res) {
    return state = res;
  }).catch(function (e) {
...
var changeModalState = function changeModalState(state) {
  var photoSize = document.querySelector('#size'),
      photoMaterial = document.querySelector('#material'),
      photoOptions = document.querySelector('#options'),
      photoPromoCode = document.querySelector('.promocode'); //   photoPrice = document.querySelector('.calc-price').innerHTML;

  function bindActionToElems(event, elem, prop) {
    elem.addEventListener(event, function () {
      elem.value == 'IWANTPOPART' ? state[prop] = true : state[prop] = false;
      state[prop] = elem.value; // switch(item.nodeName) {
      //     case 'SPAN' :
      //         state[prop] = i;
      //         break;
      //     case 'INPUT' :
...
/* harmony import */ var _services_request__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! ../services/request */ "./src/js/services/request.js");
...
var drop = function drop() {
  ...
  function highlight(item) {
    // в аргумент передаем элемент который хотим подсветить
  ...
  ['dragenter', 'dragover'].forEach(function (eventName) {
    // берем 2 события
    fileInputs.forEach(function (input) {
      // берём все файловые инпуты с которыми будем работать
      input.addEventListener(eventName, function () {
        return highlight(input);
      }, false); // на каждый инпут навешиваем событие
    });
  });
    ...
    input.addEventListener('drop', function (e) {
      ...
      if (input.closest('.main')) {
        ...
        _toConsumableArray(input.files).forEach(function (file) {
          ...
          Object(_services_request__WEBPACK_IMPORTED_MODULE_12__["postData"])('assets/server.php', formData).then(function (res) {
...
var filter = function filter(menuSelector, itemsSelector, contentWrapper, contentItems, text) {
...
var modals = function modals() {
  ...
  function openByScroll(selector) {
    window.addEventListener('scroll', function () {
      ...
      if (!btnPressed && document.documentElement.scrollTop + document.documentElement.clientHeight >= scrollHeight) {
...
var scrolling = function scrolling(upSelector) {
  ...
  var links = document.querySelectorAll('[href^="#"]'),
      // finds all links begin with #
  speed = 0.3; // скорость прокрутки 

  links.forEach(function (link) {
    // перебераем каждую ссылку
    link.addEventListener('click', function (event) {
      ...
      var heightTop = document.documentElement.scrollTop,
          // записываем сколько пролистал пользователь 
      hash = this.hash,
          // значение href у ссылки на которую кликнул пользователь
      toBlock = document.querySelector(hash).getBoundingClientRect().top,
          // как высоко/низко находиться якорь 
      start = null;
      requestAnimationFrame(step);
      debugger; // запускаем анимацию

      function step(time) {
        // создаем функцию для нашей анимации(аргумент тайм - это время прошедшее с момента начала загрузки страицы в миллисекундах)
        if (start === null) {
          debugger; // проверяем если функция запускается первый раз

          start = time;
          debugger; // если да, то присваеваем время прошедшее с момента начала загрузки страицы
        }

        debugger;
        var progress = time - start,
            // сколко времени прошло с запуска функции, как быстро она выполняется (это для установки плавного скролла )
        r = toBlock < 0 ? Math.max(heightTop - progress / speed, heightTop + toBlock) : Math.min(heightTop + progress / speed, heightTop + toBlock);
        debugger; // в первом условии проверяем куда делается скролл вверх/вниз 

        document.documentElement.scrollTo(0, r);
        debugger; // плавно скролим

        if (r != heightTop + toBlock) {
          debugger; // рекурсивно вызываем анимацию пока не дойдем до нужного элемента
          ...
          debugger;
          location.hash = hash;
          debugger; // записываем в поисковую строку #id
        }
      }
    });
  }); // Pure js scrolling
